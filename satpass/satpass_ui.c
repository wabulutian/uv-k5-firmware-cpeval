...
void Satpass_UI_DrawCircle(int centerx, int centery, int radius) 
{
	...
	for (int i = 1; i < 180; i ++) 
    {
		Satpass_UI_DrawLine(centerx + radius * 1.4f * cosLUT2deg[i], centery + radius * sinLUT2deg[i], 
...
{
    Satpass_UI_DrawCircle(RADAR_CENTER_X, RADAR_CENTER_Y, 15);
	Satpass_UI_DrawCircle(RADAR_CENTER_X, RADAR_CENTER_Y, 7);
	Satpass_UI_DrawLine(RADAR_CENTER_X - 21, RADAR_CENTER_Y, RADAR_CENTER_X + 21, RADAR_CENTER_Y, true);
	Satpass_UI_DrawLine(RADAR_CENTER_X, RADAR_CENTER_Y - 15, RADAR_CENTER_X, RADAR_CENTER_Y + 15, true);
}
...
void Satpass_UI_DrawRadar(st_PassInfo pass, uint16_t progress) 
{
    ...
    // Track
    for (i = 1; i < pass.totalFreqNum; i ++)
    {
        uint16_t az0 = pass.satAzimuthList[i-1];
        uint8_t el0 = pass.satElevationList[i-1];
        uint16_t satx0 = RADAR_CENTER_X + (el0 / 6) * cosLUT2deg[az0 / 2] * 1.4f;
	    uint16_t saty0 = RADAR_CENTER_Y + (el0 / 6) * sinLUT2deg[az0 / 2];

        uint16_t az1 = pass.satAzimuthList[i];
        uint8_t el1 = pass.satElevationList[i];
        uint16_t satx1 = RADAR_CENTER_X + (el1 / 6) * cosLUT2deg[az1 / 2] * 1.4f;
	    uint16_t saty1 = RADAR_CENTER_Y + (el1 / 6) * sinLUT2deg[az1 / 2];

        Satpass_UI_DrawLine(satx0, saty0, satx1, saty1, true);
    }

    // Current position
    az = pass.satAzimuthList[progress];
    el = pass.satElevationList[progress];
...
void Satpass_UI_DrawRssi(int dbm, uint8_t s, double enable)
{
    for (int i = 0; i < 51; i ++) {
		if (i % 5 == 0) {
			gFrameBuffer[2][i + RSSI_METER_PAD_LEFT] |= 0b00110000;
		} else {
			gFrameBuffer[2][i + RSSI_METER_PAD_LEFT] |= 0b00100000;
		}
	}
	...
	if (enable) {
        ...
        for (int i = 0; i < s + 1; i ++) {
		for (int j = 0; j < 5; j ++) {
			gFrameBuffer[2][i * 5 + j + RSSI_METER_PAD_LEFT] |= 0b00001110;
		}
	}
	} else {
		sprintf(String, "------- --");
...
void Satpass_UI_DrawStatusBar(ModulationType modulation, BK4819_FilterBandwidth_t bw, uint8_t battLevel)
{
	...
	sprintf(String, "%s %s",
			...
			bwNames[bw]);
	UI_PrintStringSmallest(String, 77, 2, true, true);
}